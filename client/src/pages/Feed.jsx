        ...
        <div className="modal-overlay">
          <div className="modal-container">
            ...
            <div className="modal-preview">
                <img src={savingPin.image_url} alt="Pin preview" className="modal-preview-img" />
                <div className="modal-preview-info">
                    <h3 className="modal-preview-title">{savingPin.space_object}</h3>
                    <p className="modal-preview-desc">{savingPin.mission_name || 'NASA Cosmos'}</p>
                </div>
            </div>
            
            {saveError && <p className="modal-error">{saveError}</p>}
            {saveSuccess && <p className="modal-success">✨ Successfully saved! ✨</p>}

            {!saveSuccess && (
                <div className="modal-board-list">
                    {boards.length === 0 ? (
                        <p className="modal-board-empty">You have no boards. Create one in the Boards tab!</p>
                    ) : (
                        boards.map(board => (
                            <div 
                                key={board.board_id} 
